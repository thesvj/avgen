...
contract is not a local decision. Every subsystem imports these, so please open
an issue and discuss it first, rather than folding the change into whatever
happens to need it.
...
another. The import direction rules in `CONTRIBUTING.md` are what keep it that
way. The boundaries are:
